...
void generate_zipf_access(GAddr access[], GAddr data[], int iter, double zipfa, unsigned int* seedp){
  int zipfPara_n = STEPS;
  double zipfPara_a = zipfa;
  // unsigned int zipfPara_lfsrSeed = rand_r(seedp);
  unsigned int zipfPara_lfsrSeed = 0;
  unsigned int zipfPara_reqNum = iter;

  double reciPowSum_res = reciPowSum(zipfPara_a, zipfPara_n);
  // printf("check: reciPowSum_res = %.2lf\n", reciPowSum_res);
  double freqSum = 0;
  double* freq_arr = (double*)malloc(sizeof(double) * zipfPara_n);
  double* freq_sum_arr = (double*)malloc(sizeof(double) * zipfPara_n);
  for(int i = 0; i < zipfPara_n; i++){
      int x = i + 1;
      double tmpFreq = 1 / (pow(x, zipfPara_a) * reciPowSum_res); 
      freqSum += tmpFreq;
      freq_arr[i] = tmpFreq;
  }
  // printf("check: freqSum = %.2lf (should be equal to 1)\n", freqSum);

  //for binary search
  freq_sum_arr[0] = freq_arr[0];
  for(int i = 1; i < zipfPara_n; i++){
      freq_sum_arr[i] = freq_sum_arr[i - 1] + freq_arr[i];
  }
  // printf("check: freq_sum_arr[zipfPara_n - 1] = %.2lf (should be equal to freqSum) \n", freq_sum_arr[zipfPara_n - 1]);
  // printf("info: binary research preperation ends.\n");

  double* freqCheck_arr = (double*)calloc(sizeof(double), zipfPara_n);

  char resFile[128];
  sprintf(resFile, "./a%0.2f_n%u_rN%d_sd%u.txt", \
  ...
  for(int i = 0; i < zipfPara_reqNum; i++){
    ...
    unsigned int key = 0;
    while(validFlag == 0){
      double tmpFreq = 1.0 * rand_r(seedp) / RAND_MAX;
      // if(i < 10){
      //     printf("reqCount=%d, tmpFreq=%.2lf\n", i, tmpFreq);
      // }
      // binary research
  ...
  //     freqCheckRes_out << ((double)(freqCheck_arr[i])) / zipfPara_reqNum << "\t" << freq_arr[i] << endl;
  // }
...
void Init(GAlloc* alloc, GAddr data[], GAddr access[], bool shared[], int id,
  ...
  if(zipfa != 0){
    generate_zipf_access(access, data, ITERATION, zipfa, seedp);
  }
  else{
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    } else if (strcmp(argv[i], "--zipfa") == 0) {
      zipfa = (double) atof(argv[++i]);
    }
  ...
  if(zipfa != 0){
    if(space_locality != 0 || time_locality != 0 || shared_ratio != 100){
      printf("zipfa != 0 only support space_locality = 0, time_locality = 0, and shared_ratio = 100");
      exit(0);
    }
  }
  /******** MY CODE ENDS ********/
  ...
  if (is_master) {
    ...
    result << zipfa << "," << no_node << "," << agent_stats_inst.num_4_sys_thread << "," << no_thread << "," << remote_ratio << ","
    ...
    epicLog(
        ...
        "zipfa: %.2lf, no_node: %d, no_sys_thread: %d, no_app_thread: %d, remote_ratio: %d, shared_ratio: %d, read_ratio: %d, space_locality: %d, "
        "time_locality: %d, op_type = %d, memory_type = %d, item_size = %d, "
        "total_throughput: %ld, avg_throuhgput:%ld, avg_latency:%ld, cache_th = %f\n\n",
        zipfa, no_node, agent_stats_inst.num_4_sys_thread, no_thread, remote_ratio, shared_ratio, read_ratio,
