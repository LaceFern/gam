  ...
  if (likely(IsLocal(wr->addr))) {
    ...
    for (GAddr i = start_blk; i < end;) {
      ...
      if (unlikely(s == DIR_DIRTY)) {
        ...
        agent_stats_inst.add_time_tag_4at("request node: call Read -> submit gmem_Hit_dir_M request");
      } else {
  ...
  if (likely(IsLocal(wr->addr))) {
    ...
    for (GAddr i = start_blk; i < end;) {
      ...
      if (state == DIR_DIRTY || state == DIR_SHARED) {
        ...
        if(state == DIR_DIRTY){
          agent_stats_inst.add_time_tag_4at("request node: call Write -> submit gmem_Hit_dir_M request");
        }
        else{
          agent_stats_inst.add_time_tag_4at("request node: call Write -> submit gmem_Hit_dir_S request");
        }
      } else {
#ifdef GFUNC_SUPPORT
        if (wr->flag & GFUNC) {
...
int Worker::ProcessLocalRLock(WorkRequest* wr) {
  ...
  if (IsLocal(wr->addr)) {
    ...
    if (state == DIR_DIRTY) {
      ...
      directory.ToToShared(entry, rc);
      SubmitRequest(cli, lwr, ADD_TO_PENDING | REQUEST_SEND);
      agent_stats_inst.add_time_tag_4at("request node: call RLock -> submit gmem_Hit_dir_M request");
    } else {
      int ret;
      if (entry) {
...
int Worker::ProcessLocalWLock(WorkRequest* wr) {
  ...
  if (IsLocal(wr->addr)) {
    ...
    if (DIR_DIRTY == state || DIR_SHARED == state) {
      ...
      if(state == DIR_DIRTY){
        agent_stats_inst.add_time_tag_4at("request node: call WLock -> submit gmem_Hit_dir_M request");
      }
      else{
        agent_stats_inst.add_time_tag_4at("request node: call WLock -> submit gmem_Hit_dir_S request");
      }
