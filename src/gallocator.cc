...
#else
  WorkRequest wr{ };
  wr.glb_thread_id = thread_id + GLB_BIAS;
  wr.op = READ;
  wr.flag = flag;
  wr.size = count;
  wr.addr = GADD(addr, offset);
...
void GAlloc::UnLock_with_thread_id(int thread_id, const GAddr addr, const Size count) {
  Lock_with_thread_id(thread_id, UNLOCK, addr, count);
}