...
Client* Server::NewClient(bool isMaster, const char* rdmaConn) {
  try {
    epicLog(LOG_WARNING, "Server::NewClient starts");
    Client* c = new Client(resource, isMaster, rdmaConn);
    uint32_t qp = c->GetQP();
    epicLog(LOG_WARNING, "Server::NewClient ends, qpn = %d", qp);
...
// void Server::ProcessRdmaRequest(ibv_wc& wc, Worker* w) {
//   void *ctx;
//   Client *cli;
//   uint32_t immdata, id;
...
//       WorkRequest* wr = w->GetPendingWork(ntohl(wc.imm_data));
...
void Server::ProcessRdmaRequest(entry_4_wq& entry) {

  agent_stats_inst.parse_starting_point = std::chrono::system_clock::now();
  ...
  agent_stats_inst.put_valid_qpn(wc.qp_num);
  ...
  switch (wc.opcode) {
    ...
    case IBV_WC_RECV: {
      ...
      while (consumed_len < wc.byte_len) {
        ...
        if (wr->Deser(data + consumed_len, len)) {
          ...
          agent_stats_inst.add_starting_point_4debug_q(wr->addr, entry.starting_point);
          agent_stats_inst.add_ending_point_4debug_q(wr->addr, entry.ending_point, s);
    ...
    case IBV_WC_RECV_RDMA_WITH_IMM: {
      ...
      ProcessRequest(cli, ntohl(wc.imm_data), entry);
      //resource->ClearSlot(wc.wr_id);
      int n = resource->PostRecvSlot(wc.wr_id);
      //epicAssert(n == 1);
      break;
