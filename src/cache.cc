...
//					}
      ...
      //to intermediate state
      if (READ == wr->op) {
        agent_stats_inst.add_time_tag_4at("request node: call Read -> submit cache_Miss request");
       ...
       // time_stamp_6 = get_time();
        ...
        agent_stats_inst.add_time_tag_4at("request node: call Write -> submit cache_Miss request");
#ifdef SELECTIVE_CACHING
      if(lwr->flag & NOT_CACHE) {
        GAddr gs = i > start ? i : start;
        void* ls = (void*)((ptr_t)wr->ptr + GMINUS(gs, start));
...
#endif
    if(wr->op == RLOCK){
      agent_stats_inst.add_time_tag_4at("request node: call RLock -> submit cache_Miss request");
    }
    else{
      agent_stats_inst.add_time_tag_4at("request node: call WLock -> submit cache_Miss request");
    }
    wr->is_cache_hit_ = false;
    WorkRequest* lwr = new WorkRequest(*wr);
