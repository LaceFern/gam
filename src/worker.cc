...
/******** MY CODE STARTS ********/
  ...
  for(int i = 0; i < agent_stats_inst.num_4_sys_thread; i++){
    this->st[i] = new thread(StartService, this, i);
  }
/******** MY CODE ENDS ********/
/***********************************/
...
/***********************************/
/******** MY CODE STARTS ********/
void Worker::enqueue_wc(entry_4_wq& entry, int sys_thread_id){
  std::lock_guard<std::mutex> lock(mtx_4_waiting_queue[sys_thread_id]);
  entry.queue_size = get_waiting_queue_size(sys_thread_id);
  waiting_queue[sys_thread_id].push(entry);
}

std::tuple<bool, entry_4_wq> Worker::dequeue_wc(int sys_thread_id){
  std::lock_guard<std::mutex> lock(mtx_4_waiting_queue[sys_thread_id]);
  bool is_empty = 0;
  entry_4_wq entry;
  
  is_empty = is_waiting_queue_empty(sys_thread_id);
  if(!is_empty){
    entry = waiting_queue[sys_thread_id].front();
    waiting_queue[sys_thread_id].pop();
    entry.ending_point = std::chrono::system_clock::now();
    entry.sys_thread_id = sys_thread_id;
  }
  return std::make_tuple(is_empty, entry);
}

int Worker::get_waiting_queue_size(int sys_thread_id){
  return waiting_queue[sys_thread_id].size();
}

bool Worker::is_waiting_queue_empty(int sys_thread_id){
  return waiting_queue[sys_thread_id].empty();
};
...
void Worker::StartQueue(Worker* w) {
  ...
  while (likely(!eventLoop->stop)) {
    ...
    for (int i = 0; i < ne; ++i) {
      ...
      entry.wc = wc[i];
      w->enqueue_wc(entry, rand() % agent_stats_inst.num_4_sys_thread);
    }
  }
  aeDeleteEventLoop(w->el);
}

void Worker::StartService(Worker* w, int sys_thread_id) {
  epicLog(LOG_WARNING, "Worker Start service!!!");
  while(1){
    auto result = w->dequeue_wc(sys_thread_id);
    if(!(std::get<0>(result))){
