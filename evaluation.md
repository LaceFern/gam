...
9. 为了减少测试所需的开辟的gmem空间，限制了非共享区域的地址数量，但理论上不应该这么受限？

10. 为什么write操作gam的req节点没有统计信息？

11. 把 MAX_SYS_THREAD 宏从12增加到了24

12. int local_addr_num = STEPS * 0.125; 表示每机都会开出（本地/远程）1GB空间用于非共享访问，8机合起来可以开出8GB用于非共享访问的空间；此时如果cache 8GB数据，那么cache命中率有（1GB+8GB）/8GB≈100%？这不合适。不如这样，论文里可以说每节点提供了1GB非共享gmem空间，1GB共享gmem空间，workload总计16GB。每节点的非共享空间各节点独占使用，而共享空间可以被所有节点访问，单节点cache大小为4GB，即每个节点能够缓存约50%的共享gmem空间的数据