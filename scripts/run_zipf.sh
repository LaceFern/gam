...
bin=`dirname "$0"`
bin=`cd "$bin"; pwd`
SRC_HOME=$bin/../test_cc_highpara_zipf
slaves=$bin/slaves
log_file=$bin/log
...
run() {
    ...
    do
    	...
    	echo "$SRC_HOME/benchmark --zipfa $zipfa --no_sys_thread $no_sys_thread --is_cache $is_cache --cache_rw $cache_rw --is_request $is_request --request_rw $request_rw --is_master $is_master --ip_master $master_ip --ip_worker $ip --no_node $node --port_worker $port --port_master $master_port --cache_th $cache_th --result_file $result_file --no_thread $thread --remote_ratio $remote_ratio --shared_ratio $shared_ratio --read_ratio $read_ratio --space_locality $space_locality --time_locality $time_locality --op_type $op_type"
...
result_file=$bin/results_20240307_3

read_range="50"
thread_range="8"
# thread_range="32 8 16"
# zipfa_range="2 0 0.99"
zipfa_range="0.99"

# sys_thread_range="1 4 8 16"
sys_thread_range="1 2 4"
is_cache="0"
cache_rw="0"
is_request="0"
request_rw="0"
