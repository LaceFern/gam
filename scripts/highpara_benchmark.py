...
base = "/home/zxy/nfs/DSM_prj/gam_20240403"
...
master_machine = "192.168.189.14"
master_ip = "10.0.0.8"

requester_machine = "192.168.189.13"
requester_ip = "10.0.0.7"

cache_machine = "192.168.189.7"
cache_ip = "10.0.0.1"
...
output_directory = "/home/zxy/gam_result_2.1"
...
remote_ratio = [0]
sharing_ratio = [5, 25, 50, 75, 100]
bench_thread = [8]
sys_thread = [1]

# remote_ratio = [88]
# sharing_ratio = [0, 25, 50, 75, 100]
# bench_thread = [4, 8]
# sys_thread = [1]
...
def master_run(ssh, program, bench_thread, sys_thread, sharing_ratio, remote_ratio, output_dir, node_num):
    stdin, stdout, stderr = ssh.exec_command(
        "cd {0}/build && ./{1} "
        ...
        "--cache_th {6} --result_dir {5} --no_thread {6} --remote_ratio {9} --shared_ratio {8} "
        "--read_ratio 50 --space_locality 0 --time_locality 0 --op_type 2".format(base, program, sys_thread, master_ip, master_ip, output_dir, bench_thread, node_num, sharing_ratio, remote_ratio)
    )
    str1 = stdout.read().decode('utf-8')
    str2 = stderr.read().decode('utf-8')
...
def requester_run(ssh, program, bench_thread, sys_thread, sharing_ratio, remote_ratio, output_dir, node_num):
    stdin, stdout, stderr = ssh.exec_command(
        "cd {0}/build && ./{1} "
        "--no_sys_thread {2} --is_cache 0 --cache_rw 0 --is_request 1 --request_rw 0 --is_master 0 " 
        "--ip_master {3} --ip_worker {4} --no_node {7} --port_worker 1234 --port_master 1231 "
        "--cache_th {6} --result_dir {5} --no_thread {6} --remote_ratio {9} --shared_ratio {8} "
        "--read_ratio 50 --space_locality 0 --time_locality 0 --op_type 2".format(base, program, sys_thread, master_ip, requester_ip, output_dir, bench_thread, node_num, sharing_ratio, remote_ratio)
    )
    str1 = stdout.read().decode('utf-8')
    str2 = stderr.read().decode('utf-8')
...
def cache_run(ssh, program, bench_thread, sys_thread, sharing_ratio, remote_ratio, output_dir, node_num):
    stdin, stdout, stderr = ssh.exec_command(
        "cd {0}/build && ./{1} "
        "--no_sys_thread {2} --is_cache 1 --cache_rw 1 --is_request 0 --request_rw 0 --is_master 0 " 
        "--ip_master {3} --ip_worker {4} --no_node {7} --port_worker 1234 --port_master 1231 "
        "--cache_th {6} --result_dir {5} --no_thread {6} --remote_ratio {9} --shared_ratio {8} "
        "--read_ratio 50 --space_locality 0 --time_locality 0 --op_type 2".format(base, program, sys_thread, master_ip, cache_ip, output_dir, bench_thread, node_num, sharing_ratio, remote_ratio)
    )
    str1 = stdout.read().decode('utf-8')
    str2 = stderr.read().decode('utf-8')
...
def other_run(ssh, program, bench_thread, sys_thread, sharing_ratio, remote_ratio, output_dir, node_num, self_ip):
    stdin, stdout, stderr = ssh.exec_command(
        "cd {0}/build && ./{1} "
        "--no_sys_thread {2} --is_cache 0 --cache_rw 0 --is_request 0 --request_rw 0 --is_master 0 " 
        "--ip_master {3} --ip_worker {4} --no_node {7} --port_worker 1234 --port_master 1231 "
        "--cache_th {6} --result_dir {5} --no_thread {6} --remote_ratio {9} --shared_ratio {8} "
        "--read_ratio 50 --space_locality 0 --time_locality 0 --op_type 2".format(base, program, sys_thread, master_ip, self_ip, output_dir, bench_thread, node_num, sharing_ratio, remote_ratio)
    )
    str1 = stdout.read().decode('utf-8')
    str2 = stderr.read().decode('utf-8')
    # print(str1)
    ...
    for b_i in bench_thread:
        for s_i in sys_thread:
            for sr_i in sharing_ratio:
                for rr_i in remote_ratio:
                    t1 = threading.Thread(target=master_run,
                                            args=(ssh_master, program_name, b_i, s_i, sr_i, rr_i, output_directory, 3+len(ssh_others)))

                    t2 = threading.Thread(target=requester_run,
                                            args=(ssh_requester, program_name, b_i, s_i, sr_i, rr_i, output_directory, 3+len(ssh_others)))

                    t3 = threading.Thread(target=cache_run,
                                            args=(ssh_cache, program_name, b_i, s_i, sr_i, rr_i, output_directory, 3+len(ssh_others)))

                    tother_list = [threading.Thread(target=other_run,
                                            args=(ssh_others[i], program_name, b_i, s_i, sr_i, rr_i, output_directory, 3+len(ssh_others), other_ip[i]))  for i in range(len(ssh_others))]
                    t1.start()
                    time.sleep(2)
                    t2.start()
                    time.sleep(1)
